...
int foreach_find_item(void *void_item_pointer, void *args) {
  ...
  char *query_strings[] = {shuffle->search_data[0], shuffle->search_data[1],
                           shuffle->search_data[2], shuffle->search_data[3]};
  char *item_strings[] = {item->data.name, item->data.surname, item->data.email,
                          item->data.phone};
